...
#A chart that shows the relationship between two quantitative variables (Alternate)

```{r}
ggplot(df, aes(x = `Stress Level`, y = `Sleep Duration`, color = `Quality of Sleep`)) +
...
Lots of debugging with GPT here. Had a hard time structuring the hierarchy needed.
...
sun_burst_all <- bind_rows(gender_nodes, sun_burst)
...
plot_ly(
  ...
  textfont = list(size = 16, color = "#ffffff")  
) %>%
  layout(
    title = list(
      text = "<b>Distribution of Sleep Disorders by Gender</b>",
    ...
    margin = list(t = 100), 
    sunburstcolorway = c("#4575b4", "#d73027") 
  )
```
